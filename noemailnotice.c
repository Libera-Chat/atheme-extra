...
static void user_identify_notice(struct hook_user_identify *hdata)
{
	...
	{
		notice(nicksvs.nick, hdata->u->nick, "WARNING: Your NickServ account does not have a valid email address set.");
		notice(nicksvs.nick, hdata->u->nick, "Should you forget your password it may not be possible to recover your acccount.");
		notice(nicksvs.nick, hdata->u->nick, "For help setting an email address, see \2/msg NickServ HELP SET EMAIL\2.");
		notice(nicksvs.nick, hdata->u->nick, "Should you need more assistance you can /join #freenode to find network staff.");
	}
}
